...
form.addEventListener('submit', async e => {
    ...
    if (coinSelected === '' && cryptoSelected === '') {
        coinInfo.innerHTML = '<p class="error">Seleccione una moneda y una cryptomoneda.</p>';
        return; 
    } 
    
    if (coinSelected === '') {
        coinInfo.innerHTML = '<p class="error">Seleccione una moneda.</p>';
        return; 
    } 
    
    if (cryptoSelected === '') {
        coinInfo.innerHTML = '<p class="error">Seleccione una cryptomoneda.</p>';
        return; 
    }
    ...
    try {
        ...
        coinInfo.innerHTML = `<p class="error">Hubo un error al conectar con la API.</p>`;
    }
})